...
- **T4.2** First processors: Mill (Sunwheat -> Sunflour), Preserve Pot (Glowberry -> Glowjam); queue-based, timestamp timers, collect on tap (no dragging goods - inputs auto-pull from storage). PM PROPOSAL 2026-07-14 (from the review triage, AWAITING OWNER APPROVAL - not yet the sequence of record): build ONE complete chain end to end first (Mill -> flour -> Bakery -> bread + one flour-plus-crop combo recipe) before processor breadth, the Preserve Pot waiting on the vertical-slice verdict.

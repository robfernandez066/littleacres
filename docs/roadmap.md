...
- **T3.4** Camera: pinch zoom + pan within unlocked bounds; snap-back at edges; UI stays fixed. (Owner emphasis 2026-07-12: panning must be SMOOTH, and bounds tight - enough room to arrange decor/buildings, never oceans of dead space. Positioned late-roadmap as a package with land expansion T3.3 - whichever moves first pulls the other.) REAFFIRMED 2026-07-14 with guardrails (owner decision; supersedes the PM review-triage pinch-zoom rejection): optional two-finger pinch-zoom + pan on the farm scene only - an in-game camera gesture, never browser page zoom; HUD stays fixed; core actions (plant, harvest, orders) never require zoom; an active pinch suppresses accidental plant/harvest/tap gestures; a reliable reset/recenter-to-home behavior; the one-hand default view stays fully playable; text readability is T3.23's concern, not zoom's.
...
- **REVIEW-INTAKE FIXES (triaged and BLESSED 2026-07-14 - see decisions):** **T3.17** save/trophy integrity, P0 (cap counts purchasable frames only, shared definition across buy/validation; last-known-good backup slot; invalid saves never overwritten, rejected blob stashed for recovery; reload regression test) -> **T3.19** weekly quests v1.1 (featured crop from unlocked only; exhaustive per-crop targets incl. Dewmelon/Sagesprig; rollover on scene tick + resume; completed-unclaimed rewards AUTO-GRANTED before reset with a queued notice - owner decision, no inbox; recalibrated week-shaped targets - approach approved, concrete numbers proposed in the prompt for explicit approval) -> **T3.18** trophy shelf (Shed trophy section; place/store like decor). Then, sequenced to avoid file conflicts: **T3.20** offline summary on PWA foreground (threshold hidden-time; runs after the integrity commit - shares gameState.ts), **T3.21** staged audio loading (block on atlas+SFX only; stream music/ambient after farm start), **T3.22** clarity batch (L6 system-unlock card, premium flavor line restored, two-chest tier by order value, themed letterbox background - an interim improvement only; the responsive safe-area/overscan work stays with T3.3/T3.4), and the CI test+lint gate (deploy.yml only, anytime). **T3.23** seed-bar scrolling row (4-5 full-size cards, selected centered, locked teaser kept) also carries the minimum mobile readability + touch-target baseline (44-48 CSS px targets, 14-16 CSS px body text) - kept separate per owner; timing DECIDED 2026-07-14 - runs with the P2 batch, before gate wrap and wave 3.
- **WAVE 3 CANDIDATE SET (owner 2026-07-14):** crop mastery (T3.5), storage caps (T3.2), and restoration chapter v1 (NEW - pillar 5 made playable; before it can be scheduled the PM must define how it differs from T3.3's land-expansion/region-art/mere-glow scope). The reward-only Mine v1 is dropped (see Parked concepts).
...
- **T4.2** First processors: Mill (Sunwheat -> Sunflour), Preserve Pot (Glowberry -> Glowjam); queue-based, timestamp timers, collect on tap (no dragging goods - inputs auto-pull from storage). APPROVED 2026-07-14 (owner): build ONE complete chain end to end first (Mill -> flour -> Bakery -> bread + one flour-plus-crop combo recipe) before processor breadth, the Preserve Pot waiting on the vertical-slice verdict. THEME DIRECTIVE (owner, 2026-07-14, applies to all of Phase 4): production buildings and products keep the magical/fantasy identity - prefer magical variants in naming, art, and flavor (Sunwheat -> Sunflour is the pattern; enchanted mill, moonlit bakery - never mundane farm industry).
...
- **Recurring villager cast** (external review, 2026-07-14): a small named cast (mayor, baker, healer, merchant, festival organizer) with portrait + one-line request + short delivery reaction on order cards - continuity and attachment without dialogue trees. On-theme (GDD: flavor text carries the story) but needs a portrait art pipeline and a card redesign - revisit with T4.4 order board v3. Near-term nod: T3.22 restores the premium flavor lines the cards already have but stopped showing.

...
TRACK C (coder, parallel small tasks, in order): 1. T2.2 smart replant (promoted; pure code, biggest repeated friction) -> 2. T2.16 save durability -> 3. T2.5 Moondust earning + Radiant variant (promoted; ends the dead-currency promise, adds the only near-term surprise mechanic) -> 4. T2.17 SW update toast -> 5. T2.11 order-card layout -> 6. T2.14 crop tap countdown -> 7. T2.12 skip escalation (MODIFIED: 60s cap).

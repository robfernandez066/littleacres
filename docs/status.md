...
**Active task:** T2.9 + T2.9a + T2.9b awaiting the combined USER TEST walk, then one commit. T2.9 (rails): review passed, user-run verify green (167 tests), PM stale-comment fix landed. T2.9a (completion celebration): review passed; accepted deviation: banner uses BANNER_TO_CARD_MS. T2.9b (card overflow fix + "Let's Go!" button, from user screenshot): review passed - per-kind card geometry (700x620 tutorial / 700x480 unlock restored on every show), button on the backdrop-tap dismiss path; coder verified clearances >= 24px via measured getBounds in-browser (exemplary verification); 170 tests. Commit message delivered (T2.9+a+b combined); after commit + Actions green, next up is T1.12b (music slider remap: full track = 0.4 gain, today's 0.2 default displays at 50%, no migration - prompt already delivered, FRESH session, Sonnet). NOTE: PM sandbox mount served corrupt copies of ALL fresh coder writes during review (T0.3 precedent, sharper diagnosis this time): the mount clips fresh content to each file's OLD byte length - NUL-padded when the new file is shorter, silently truncated when longer, and byte counts alone cannot detect the latter. PM ran a partial verification in a repaired scratch copy (63/63 tests pass across 7 files incl. the new onboarding chain tests; gameState.test.ts and 4 src files unverifiable there). Direct Read of every affected file confirmed the repo itself is intact. Authoritative test/build/lint run belongs to the user's machine. Standing PM rule: review fresh coder work via direct reads only; the outputs-folder bridge works for NEW files if sandbox execution is ever essential.
...
- Audio session (bundled with the T2.13 mockup session): coin fly-in SFX audition (PM sources 5-6 candidates with rates, user picks) PLUS an SFX gap audit - known suspects: expansion purchase reuses the plain tap sound, no ambient loop (GDD lists birds/wind chimes/mere water), and the WYWA Confirm button will need a sound once T2.10 lands

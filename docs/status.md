...
- **T3.20+T3.20a and T3.21 SHIPPED 2026-07-14** (foreground WYWA; staged audio - farm paints before the ~15.8MB of music). **T3.22+a SHIPPED. T3.23+T3.23a reviewed PASS - in USER TEST** (scroll strip + stuck-drag fix; one combined commit when green). Tests 345.
...
1. T3.23+T3.23a user test -> combined commit -> decor_well atlas regen (own commit); then the tiny CI gate closes the P2 batch -> gate wrap-up
...
- Seed-bar scroll strip + readability bump feel on a real phone (after T3.23 ships)
- WYWA foreground-summary threshold feel (after T3.20)

## Waiting on user (whenever convenient)

- Atlas regen pending: decor_well.png master touched up (owner, 2026-07-14). AFTER T3.23 is committed: `npm run pack:atlas`, verify the well in game, commit the regenerated atlas files alone.

...
/**
 * Count connected components (4-neighbour) of `alpha > threshold` whose size
 * exceeds `minSize`. Used to prove the farmhouse shadow is ONE continuous shape
 * rather than fragments.
 */
function majorAlphaComponents(image, threshold, minSize) {
  const { width, height, data } = image.bitmap;
...
/**
 * Authored-shadow override (T3.28): if tools/shadow-overrides/<name>_shadow.png
 * (+ .json) exists, use that hand-authored shadow instead of generateCastShadow.
 * The PNG is a FULL untrimmed logical canvas (transparent bg, pure-black RGB,
 * authored alpha, shadow only). Its JSON pins the logical size, the conceptual
 * 256x256 farmhouse registration rect, and the anchor (the rect's bottom-center,
 * which maps to farmhouseImage.x/y at runtime). We validate all of that, force
 * pure-black RGB, then trim to opaque bounds exactly like generateCastShadow so
 * it packs with identical trim metadata (spriteSourceSize / sourceSize).
 * Returns null when no override exists, so the generic generator stays the
 * fallback. Anchor is intentionally NOT re-tuned here - shape/opacity/projection
 * are whatever the authored PNG holds.
 */
async function loadAuthoredShadowOverride(name) {
  ...
  const logical = await Jimp.read(pngPath);
  const logicalWidth = logical.bitmap.width;
  const logicalHeight = logical.bitmap.height;
  if (metadata.logicalWidth !== logicalWidth || metadata.logicalHeight !== logicalHeight) {
    throw new Error(
      `${name}: authored shadow metadata says ` +
        `${metadata.logicalWidth}x${metadata.logicalHeight}, ` +
        `but PNG is ${logicalWidth}x${logicalHeight}`,
    );
  }
  if (metadata.frame !== `${name}_shadow`) {
    throw new Error(`${name}: authored shadow metadata frame mismatch`);
  }
  const rect = metadata.sourceFrameRect;
  const anchor = metadata.anchor;
  if (rect.width !== 256 || rect.height !== 256) {
    throw new Error(`${name}: farmhouse registration rect must be 256x256`);
  }
  if (anchor.x !== rect.x + 128 || anchor.y !== rect.y + 256) {
    throw new Error(`${name}: authored anchor must be the registration frame's bottom-center`);
  }
  if (
    rect.x < 0 ||
    rect.y < 0 ||
    rect.x + rect.width > logicalWidth ||
    rect.y + rect.height > logicalHeight
  ) {
    throw new Error(`${name}: registration frame lies outside the logical canvas`);
  }
  if (anchor.x < 0 || anchor.x > logicalWidth || anchor.y < 0 || anchor.y > logicalHeight) {
    throw new Error(`${name}: authored anchor lies outside the logical canvas`);
  }
  if (metadata.tuckRatio !== 0) {
    throw new Error(`${name}: authored farmhouse shadow must use zero runtime tuck`);
  }
  // Preserve authored alpha, but enforce pure-black RGB.
  const data = logical.bitmap.data;
  for (let i = 0; i < data.length; i += 4) {
    data[i] = 0;
    data[i + 1] = 0;
    data[i + 2] = 0;
  }
  const bounds = opaqueBounds(logical);
  if (bounds === null) {
    throw new Error(`${name}: authored shadow is fully transparent`);
  }
  // Authored-farmhouse shadow invariant (T3.28): the farmhouse shadow is a
  // simple, stylized ground shape (not a projected silhouette), so validation is
  // deliberately practical - its upper edge must begin ABOVE the ground anchor
  // (so the building sprite covers it and the shadow reads as emerging from
  // beneath the base), and it must be ONE connected mass. No per-post / silhouette
  // / contact-point checks. Farmhouse only; generic shadows are never affected.
  if (name === 'farmhouse') {
    if (bounds.y >= metadata.anchor.y) {
      throw new Error(
        `${name}: authored shadow begins at y=${bounds.y}, which is not above ` +
          `its ground anchor y=${metadata.anchor.y}; the shadow would render detached`,
      );
    }
    const major = majorAlphaComponents(logical, 8, 400);
    if (major !== 1) {
      throw new Error(
        `${name}: authored shadow forms ${major} major alpha>8 components; it must be ` +
          `one continuous shape`,
      );
    }
  }
  const trimmed = logical.clone().crop(bounds);
  return {
    image: trimmed,
    trim: { x: bounds.x, y: bounds.y, sourceW: logicalWidth, sourceH: logicalHeight },
  };
}

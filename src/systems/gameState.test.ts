...
describe('real migration v9 -> v10 (decorations + warehouse)', () => {
  it('a v9 save (no decorations/warehouse fields) gains both empty and migrates through to current', () => {
    ...
    // empty store to assert on at the CURRENT version is the shed.
    expect(store.getState().shedInventory).toEqual({});
    expect('warehouse' in state).toBe(false);
    expect(console.warn).not.toHaveBeenCalled();
  });
});
...
describe('decorations and stored-item validation and round-trip', () => {
  it('accepts a save with placed decorations (incl. a trophy frame) and stored items, and it survives a reload', () => {
    ...
    // DELIBERATE RE-PIN (U2a): the stored items live in `shedInventory` now -
    // `warehouse` is gone from the state shape, so a v14 save carrying one is
    // written as a raw field and arrives merged (this save has no shed mirror
    // to go stale, so the counts pass straight through).
    const stored = { decor_fence: 2, decor_mushrooms: 1 };
    const rawSaved = { ...saved, warehouse: stored } as unknown as Record<string, unknown>;
    expect(isValidState(saved, 14)).toBe(true);

    const storage = makeStorage({ [SAVE_KEY]: JSON.stringify(rawSaved) });
    const store = new GameStateStore({ storage });
    store.load();
    expect(store.getState().decorations).toEqual(saved.decorations);
    expect(store.getState().shedInventory).toEqual(stored);
  });
  ...
  it('rejects a shed record with an unknown id, a non-positive/non-integer count, or a malformed record', () => {
    const base = createDefaultState(12);
    expect(isValidState({ ...base, shedInventory: { not_a_frame: 1 } }, 12)).toBe(false);
    expect(isValidState({ ...base, shedInventory: { decor_bench: 0 } }, 12)).toBe(false);
    expect(isValidState({ ...base, shedInventory: { decor_bench: -1 } }, 12)).toBe(false);
    expect(isValidState({ ...base, shedInventory: { decor_bench: 1.5 } }, 12)).toBe(false);
    expect(isValidState({ ...base, shedInventory: 'nope' }, 12)).toBe(false);
  });

  it('rejects when non-fence placed + stored exceeds MAX_DECOR_ITEMS (50), even split across both - accepts exactly at the cap', () => {
    const base = createDefaultState(12);
    const combined = {
      ...base,
      ...
      shedInventory: { decor_barrels: 11 },
    };
    expect(isValidState(combined, 12)).toBe(false);
    combined.shedInventory = { decor_barrels: 10 };
  ...
  it('budgets are split (T3.3a2): 50 decor + 60 fences validates together, the 61st fence fails, fences never consume decor slots', () => {
    ...
    const bothAtCap = {
      ...
      // ...10 stored decor + 30 stored fences = 50 decor, 60 fences.
      shedInventory: { decor_barrels: 10, decor_fence: 30 },
    };
    expect(isValidState(bothAtCap, 12)).toBe(true);
    const extraFence = {
      ...bothAtCap,
      shedInventory: { ...bothAtCap.shedInventory, decor_fence: 31 },
    };
    expect(isValidState(extraFence, 12)).toBe(false);
  });

  it('exempts trophies from both budgets: 50 decor + all 5 trophies (mixed placed/stored) validates, 51 decor does not', () => {
    const base = createDefaultState(12);
    const placement = (frame: string) => ({ frame, x: 0, y: 0, scale: 1, flip: false });
    const atCapWithTrophies = {
      ...
      // ...10 purchasable stored + 3 trophies stored = 50 purchasable, 5 trophies.
      shedInventory: {
        decor_barrels: 10,
        trophy_moonwell: 1,
    ...
    const extraStored = {
      ...atCapWithTrophies,
      shedInventory: { ...atCapWithTrophies.shedInventory, decor_barrels: 11 },
    };
    expect(isValidState(extraStored, 12)).toBe(false);
    const extraPlaced = {
      ...atCapWithTrophies,
...
describe('buyDecoration', () => {
  it('deducts coins, increments the shed count, and persists (no placement)', () => {
    const storage = makeStorage();
    const store = new GameStateStore({ storage });
    ...
    const reloaded = new GameStateStore({ storage });
    reloaded.load();
    expect(reloaded.getState().shedInventory).toEqual(state.shedInventory);
  ...
  it('deducts moondust for a moondust-priced item', () => {
    ...
    const state = store.getState();
    expect(state.moondust).toBe(0);
    expect(state.shedInventory).toEqual({ decor_mushrooms: 1 });
    expect(state.decorations).toEqual([]);
  });

  it('stacks repeat purchases of the same frame in the shed', () => {
    const store = new GameStateStore({ storage: null });
    completeOnboarding(store);
  ...
  it('fails at MAX_DECOR_ITEMS non-fence purchasable (placed + stored combined), without mutation - fences still buyable (T3.3a2)', () => {
    const store = new GameStateStore({ storage: null });
    completeOnboarding(store);
    ...
    expect(store.getState().shedInventory).toEqual({ decor_bench: 50 });
    const snapshot = JSON.parse(store.exportSave()) as unknown;
    expect(store.buyDecoration('decor_bench')).toBe(false);
    expect(JSON.parse(store.exportSave())).toEqual(snapshot);
  ...
  it('fails at MAX_FENCES fences, without mutation - decor still buyable (T3.3a2)', () => {
    ...
    expect(store.getState().shedInventory).toEqual({ decor_fence: 60 });
    const snapshot = JSON.parse(store.exportSave()) as unknown;
  ...
  it('ignores trophies for the cap: buys at 49 purchasable + all 5 trophies, fails at 50 purchasable', () => {
    ...
    expect(store.buyDecoration('decor_bench')).toBe(false);
    expect(store.getState().shedInventory.decor_bench).toBe(50);
  });

  it('counts placed decorations toward the same cap as stored ones', () => {
    const store = new GameStateStore({ storage: null });
    completeOnboarding(store);
    store.addCoins(1_000_000);
...
describe('storeDecoration', () => {
  it('removes a placement, increments its shed count, and persists', () => {
    const storage = makeStorage();
    const store = new GameStateStore({ storage });
    completeOnboarding(store);
    store.addCoins(2000); // RE-PIN (Balance Pass v2): decor_bench 400 -> 600, so two no longer fit in 1000.
    ...
    reloaded.load();
    expect(reloaded.getState().decorations).toEqual([]);
    expect(reloaded.getState().shedInventory).toEqual({ decor_bench: 1 });
  });
  ...
  /**
   * The U2a cutover routes both halves of this round trip through the shed
   * reducers, so the placement a re-place produces has to be byte-identical to
   * the one the first place produced - for a TROPHY exactly as for a purchased
   * decoration, now that trophies are catalog items and no longer refused.
   */
  it.each(['decor_bench', 'trophy_moonwell'])(
    'place -> store -> place leaves a byte-identical placement (%s)',
    (frame) => {
      const store = new GameStateStore({ storage: null });
      completeOnboarding(store);
      store.getState().shedInventory[frame] = 1;
  ...
  it('accumulates the shed count across multiple stores of the same frame, index shift accounted for', () => {
    const store = new GameStateStore({ storage: null });
    completeOnboarding(store);
    store.addCoins(2000); // RE-PIN (Balance Pass v2): decor_bench 400 -> 600, so two no longer fit in 1000.
    store.buyDecoration('decor_bench');
...
describe('real migration v16 -> v17 (fence normalization + per-item sizing, T3.3a2)', () => {
  ...
  it('round-trips a v16 save with an oversized fence and an oversized table item: both normalize, everything else identical', () => {
    ...
    saved.coins = 777;
    // Written as a raw field: `warehouse` is not part of the state shape any
    // more (U2a), and the whole-state pin below asserts it never comes back.
    const rawSaved = { ...saved, warehouse: { decor_gnome: 2 } };
    const storage = makeStorage({ [SAVE_KEY]: JSON.stringify(rawSaved) });
    const store = new GameStateStore({ storage });
    store.load();
    expect(store.getState()).toEqual({
      ...
      // unchanged at 2 (the merge overwrites, never adds, so a mirrored item is
      // not double-counted); `warehouse` is absent from the migrated state,
      // which is why it no longer appears in this pin at all.
      shedInventory: { decor_gnome: 2 },
      decorations: [
...
describe('real migration v17 -> v18 (movable structures, T3.3s)', () => {
  ...
  it('PIXEL IDENTITY: the default anchors reproduce the canonical render positions exactly', () => {
    ...
    //                                 = (796, 512)
    //                + STRUCTURE_RENDER_OFFSETS.farmhouse (151, 227)
    //                = (947, 739)
    ...
    //
    // DELIBERATE RE-PIN (T4.12 - farmhouse only; the board is untouched): the
    // farmhouse grew to FARMHOUSE_DISPLAY_HEIGHT 576 (scale 2.25) so its base
    // spans its 2x2 footprint, and its render offset was re-derived to put the
    // base's front corner ON the footprint's front corner instead of 37px
    // above it. Both numbers now DERIVE from the knobs in config.ts:
    //   x: 128 + (128 - 118) * 2.25 = 150.5 -> 151   [was 137]
    //   y: 256 + (256 - 269) * 2.25 = 226.75 -> 227  [was 219 = 9 + 420/2]
    // where (128, 256) is the footprint diamond's front corner relative to the
    // anchor centre, (128, 256) native is the sprite's ground point, and
    // (118, 269) native is FARMHOUSE_BASE_FRONT_CORNER_NATIVE (the measured
    // intersection of the base's two front edges). So (933, 731) -> (947, 739).
    //
    ...
    expect(FARMHOUSE_POSITION).toEqual({ x: 947, y: 739 });
...
describe('real migration v30 -> v31 (Shed tooltip flag, U2b)', () => {
  it('a v30 save (no shedTipSeen field) gains shedTipSeen false, no warnings', () => {
    const saved = createDefaultState(31) as unknown as Record<string, unknown>;
    saved.version = 30;
    delete saved.shedTipSeen; // a genuine v30 save never had this field
    saved.coins = 9876;
    const storage = makeStorage({ [SAVE_KEY]: JSON.stringify(saved) });
    const store = new GameStateStore({ storage });
    store.load();
  ...
  it('a full-chain v1 save lands at the current version having not seen the tooltip', () => {
    const saved = createDefaultState(1) as unknown as Record<string, unknown>;
    delete saved.shedTipSeen;
    const storage = makeStorage({ [SAVE_KEY]: JSON.stringify(saved) });
    const store = new GameStateStore({ storage });
    store.load();
    expect(store.getState().version).toBe(33);
  ...
  it('a fresh save has not seen the tooltip either', () => {
    expect(createDefaultState(31).shedTipSeen).toBe(false);
  });

  it('shedTipSeen round-trips through export/import', () => {
    const store = new GameStateStore({ storage: null });
    store.markShedTipSeen();
    expect(store.getState().shedTipSeen).toBe(true);
    const reloaded = new GameStateStore({ storage: null });
    expect(reloaded.importSave(store.exportSave())).toBe(true);
    expect(reloaded.getState().shedTipSeen).toBe(true);
  });

  it('validation rejects a missing or non-boolean shedTipSeen', () => {
    expect(isValidState(createDefaultState(31), 31)).toBe(true);
    const missing = createDefaultState(31) as unknown as Record<string, unknown>;
    delete missing.shedTipSeen;
    expect(isValidState(missing, 31)).toBe(false);
    const wrongType = createDefaultState(31) as unknown as Record<string, unknown>;
    wrongType.shedTipSeen = 1;
    expect(isValidState(wrongType, 31)).toBe(false);
  });
});

describe('markShedTipSeen (U2b)', () => {
  it('flips shedTipSeen true and persists it', () => {
    const storage = makeStorage({});
    const store = new GameStateStore({ storage });
    store.load();
    expect(store.getState().shedTipSeen).toBe(false);
    store.markShedTipSeen();
    expect(store.getState().shedTipSeen).toBe(true);
    const persisted = JSON.parse(storage.data.get(SAVE_KEY) as string) as { shedTipSeen: boolean };
    expect(persisted.shedTipSeen).toBe(true);
  });
...
describe('claimQuest', () => {
  ...
  it('claims a trophy-reward long quest: grants the trophy into the shed (no cap check), marks longClaimed, one-time', () => {
    const saved = completedLongSave('golden_fields');
    const storage = makeStorage({ [SAVE_KEY]: JSON.stringify(saved) });
    const store = new GameStateStore({ storage });
    store.load();
    ...
    const state = store.getState();
    expect(state.shedInventory.trophy_goldscarecrow).toBe(1);
    expect(state.quests.longClaimed).toEqual(['golden_fields']);
    // One-time: a second claim fails without granting a second trophy.
    expect(store.claimQuest('golden_fields')).toBe(false);
    expect(store.getState().shedInventory.trophy_goldscarecrow).toBe(1);

    const reloaded = new GameStateStore({ storage });
    reloaded.load();
    expect(reloaded.getState().shedInventory.trophy_goldscarecrow).toBe(1);
  ...
  it('claiming a trophy at the purchasable cap survives a reload - the farm is intact, not reset (T3.17 regression)', () => {
    const saved = completedLongSave('golden_fields');
    // A v12 save, so the stored fences go in as a raw `warehouse` field and the
    // migration chain carries them into the shed (U2a) - presetting
    // `shedInventory` here would not survive v28ToV29, which rebuilds it.
    const rawSaved = {
      ...saved,
      warehouse: { decor_fence: 30 }, // already at the purchasable cap
    ...
    expect(state.shedInventory.trophy_goldscarecrow).toBe(1);
    expect(state.shedInventory.decor_fence).toBe(30);
    expect(state.quests.longClaimed).toEqual(['golden_fields']);
...
describe('buildings (T4.1, schema v23)', () => {
      ...
      expect(store.getState().buildings).toEqual([
        {
          ...
          batches: [],
          // RE-PIN (T4.8): a building is born unmirrored. (U3a: `unlockedSlots`
          // is gone from the placement - capacity lives in buildingSlotUnlocks.)
          flipped: false,
        },
      ]);
  ...
  describe('moveBuilding', () => {
    ...
    it('accepts a free anchor, persists it, and re-derives the render position', () => {
      ...
      expect(store.getState().buildings[0]).toEqual({
        ...
        row: -1,
        batches: [],
        // RE-PIN (T4.8): a move leaves the mirror flag alone. (U3a: no
        // `unlockedSlots` on the placement anymore.)
        flipped: false,
      });
      const reloaded = new GameStateStore({ storage });
      reloaded.load();
  ...
  describe('the shared structures+buildings footprint mechanism', () => {
    ...
    it('two buildings block each other (no mill special-casing)', () => {
      ...
      store
        .getState()
        .buildings.push(
          { type: 'flour_mill', col: -3, row: 0, batches: [], flipped: false },
          { type: 'flour_mill', col: 6, row: 6, batches: [], flipped: false },
        );
  ...
  describe('schema v22 -> v23 migration', () => {
    ...
    it('a fresh save starts with no buildings either', () => {
      expect(createDefaultState(25).buildings).toEqual([]);
    });
  ...
  describe('validation', () => {
    it('accepts a fresh state and a state holding a legal placement', () => {
      ...
      withMill.buildings = [{ type: 'flour_mill', col: -3, row: 0, batches: [], flipped: false }];
      expect(isValidState(withMill, 25)).toBe(true);
    });

    it('rejects a missing or non-array buildings field', () => {
      const missing = createDefaultState(25) as unknown as Record<string, unknown>;
      delete missing.buildings;
      expect(isValidState(missing, 25)).toBe(false);
      const wrongType = createDefaultState(25) as unknown as Record<string, unknown>;
      wrongType.buildings = {};
      expect(isValidState(wrongType, 25)).toBe(false);
    });

    it('rejects a placement with an unknown type or out-of-bounds coordinates', () => {
      const badType = createDefaultState(25) as unknown as Record<string, unknown>;
      badType.buildings = [{ type: 'barn', col: 0, row: 0, batches: [], unlockedSlots: 1 }];
      expect(isValidState(badType, 25)).toBe(false);

      const badCol = createDefaultState(25) as unknown as Record<string, unknown>;
      badCol.buildings = [
        { type: 'flour_mill', col: PLOT_GRID_COORD_MAX + 1, row: 0, batches: [], unlockedSlots: 1 },
      ];
      expect(isValidState(badCol, 25)).toBe(false);

      const fractional = createDefaultState(25) as unknown as Record<string, unknown>;
      fractional.buildings = [
        { type: 'flour_mill', col: 0.5, row: 0, batches: [], unlockedSlots: 1 },
      ];
      expect(isValidState(fractional, 25)).toBe(false);

      const missingRow = createDefaultState(25) as unknown as Record<string, unknown>;
      missingRow.buildings = [{ type: 'flour_mill', col: 0, batches: [], unlockedSlots: 1 }];
      expect(isValidState(missingRow, 25)).toBe(false);
    });

    it('rejects an over-cap batches array and a malformed batch (T4.2a)', () => {
      const overCap = createDefaultState(25) as unknown as Record<string, unknown>;
      ...
      overCap.buildings = [
        {
          ...
          unlockedSlots: RECIPE.slots,
        },
      ];
      expect(isValidState(overCap, 25)).toBe(false);

      const malformed = createDefaultState(25) as unknown as Record<string, unknown>;
      malformed.buildings = [
        { type: 'flour_mill', col: -3, row: 0, batches: [{ startedAt: 'soon' }], unlockedSlots: 1 },
      ];
      expect(isValidState(malformed, 25)).toBe(false);

      const nonFinite = createDefaultState(25) as unknown as Record<string, unknown>;
      nonFinite.buildings = [
        {
          type: 'flour_mill',
          col: -3,
          row: 0,
          batches: [{ startedAt: Number.NaN }],
          unlockedSlots: 1,
        },
      ];
      expect(isValidState(nonFinite, 25)).toBe(false);

      const missingBatches = createDefaultState(25) as unknown as Record<string, unknown>;
      missingBatches.buildings = [{ type: 'flour_mill', col: -3, row: 0, unlockedSlots: 1 }];
      expect(isValidState(missingBatches, 25)).toBe(false);
    });
  });
  ...
  describe('schema v23 -> v24 migration (milling batches)', () => {
    it('an EXISTING placement gains batches: [] (whole-state re-pin)', () => {
      ...
      expect(store.getState()).toEqual({
        ...
        // buildingSlotUnlocks key (the empty map ...saved already carries).
  ...
  describe('schema v24 -> v25 migration (unlockable slots)', () => {
    it('an EXISTING placement gains unlockedSlots: 1 (whole-state re-pin)', () => {
      // The owner's live save carries a dev-placed mill, so the migration has
      // to upgrade a real placement, not just an empty list.
      const saved = createDefaultState(25) as unknown as Record<string, unknown>;
      saved.version = 24;
      // A genuine v24 placement: batches, but no unlockedSlots field.
      saved.buildings = [{ type: 'flour_mill', col: -3, row: 0, batches: [] }];
      const storage = makeStorage({ [SAVE_KEY]: JSON.stringify(saved) });
      const store = new GameStateStore({ storage });
      store.load();
      expect(store.getState()).toEqual({
    ...
    it('a mill mid-run drops to one slot, stranding its extra batches (accepted loss)', () => {
      // OWNER CALL: a v24 save could have `slots` batches in flight, and the
      // flat migration locks all but the first away. Pinned because it is a
      // deliberate loss, not an oversight - if this ever needs to become a
      // batch-count floor, this is the test that has to change with it.
      const saved = createDefaultState(25) as unknown as Record<string, unknown>;
      saved.version = 24;
      saved.buildings = [
        {
          ...
          col: -3,
          row: 0,
          batches: Array.from({ length: RECIPE.slots }, () => ({ startedAt: now() })),
        },
      ];
      const storage = makeStorage({ [SAVE_KEY]: JSON.stringify(saved) });
      const store = new GameStateStore({ storage });
      store.load();
      expect(unlockedSlotsFor(store.getState(), 'flour_mill')).toBe(1);
      // The save still LOADS - the batches survive in state and the validator
      // (which caps on the def's slots, not on unlockedSlots) accepts them.
      expect(store.getState().buildings[0]!.batches).toHaveLength(RECIPE.slots);
      expect(console.warn).not.toHaveBeenCalled();
      // They are simply unreachable until the slots are bought back. Capacity
      // dropped to 1 (U3a: the map has no flour_mill key, so `unlockedSlotsFor`
      // reads the born-with 1), which locks every slot past the first.
      const views = millSlots(
        store.getState().buildings[0]!,
        RECIPE,
        now(),
        unlockedSlotsFor(store.getState(), 'flour_mill'),
      );
      expect(views.filter((view) => view.kind === 'locked')).toHaveLength(RECIPE.slots - 1);
    });
    ...
    it('RE-PIN (U3a): the slot bound moved off the placement onto buildingSlotUnlocks', () => {
      // A placement no longer carries `unlockedSlots` at all - a stray legacy
      // one is just an ignored extra property, neither required nor bounded.
      const strayField = createDefaultState(25) as unknown as Record<string, unknown>;
      strayField.buildings = [
        { type: 'flour_mill', col: -3, row: 0, batches: [], unlockedSlots: 99, flipped: false },
      ];
      expect(isValidState(strayField, 25)).toBe(true);

      // The capacity bound lives on the per-type map now: an integer in
      // [2, recipe.slots] keyed by a KNOWN building id. A 1 is the born-with
      // default and must never be written; the empty map is valid.
      const validWith = (map: unknown): boolean => {
        const s = createDefaultState(25) as unknown as Record<string, unknown>;
        s.buildingSlotUnlocks = map;
        return isValidState(s, 25);
      };
      expect(validWith({})).toBe(true);
      expect(validWith({ flour_mill: RECIPE.slots })).toBe(true);
      expect(validWith({ flour_mill: 1 })).toBe(false); // redundant born-with value
      expect(validWith({ flour_mill: 0 })).toBe(false);
      expect(validWith({ flour_mill: RECIPE.slots + 1 })).toBe(false);
      expect(validWith({ flour_mill: 1.5 })).toBe(false);
      expect(validWith({ barn: 2 })).toBe(false); // unknown building id
    });
  });
});
...
/**
 * `millStore` with every slot already bought (T4.2b-r1), for the tests that
 * are about BATCHES rather than about the lock - a mill is born with one
 * usable slot, so the multi-batch cases have to open the rest first.
 */
function fullMillStore(wheat: number, storage: SaveStorage | null = null): GameStateStore {
  const store = millStore(wheat, storage);
  while (unlockedSlotsFor(store.getState(), 'flour_mill') < RECIPE.slots) {
    expect(store.devUnlockMillSlot(0)).toBe(true);
  }
  return store;
}
...
describe('milling (T4.2a)', () => {
  ...
  describe('startMilling', () => {
    ...
    it('refuses (unmutated) when every UNLOCKED slot is busy', () => {
  ...
  describe('unlockMillSlot (T4.2b-r1)', () => {
    /** The mill's own unlock prices, in the order they are charged. */
    const COSTS = RECIPE.slotUnlockCosts;

    it('a fresh mill starts with exactly one usable slot', () => {
      const store = millStore(0);
      expect(unlockedSlotsFor(store.getState(), 'flour_mill')).toBe(1);
      // The def still declares three - what changed is how many are PAID FOR.
      expect(RECIPE.slots).toBeGreaterThan(1);
      expect(COSTS).toHaveLength(RECIPE.slots - 1);
    });

    it('charges each slot its own cost, in order, and persists', () => {
      const storage = makeStorage();
      const store = millStore(0, storage);
      store.addCoins(COSTS[0]! + COSTS[1]!);
      const before = store.getState().coins;

      expect(store.unlockMillSlot(0)).toBe(true);
      expect(store.getState().coins).toBe(before - COSTS[0]!);
      expect(unlockedSlotsFor(store.getState(), 'flour_mill')).toBe(2);

      expect(store.unlockMillSlot(0)).toBe(true);
      expect(store.getState().coins).toBe(before - COSTS[0]! - COSTS[1]!);
      expect(unlockedSlotsFor(store.getState(), 'flour_mill')).toBe(RECIPE.slots);

      const reloaded = new GameStateStore({ storage });
      reloaded.load();
      expect(unlockedSlotsFor(reloaded.getState(), 'flour_mill')).toBe(RECIPE.slots);
    });

    it('is SEQUENTIAL: slot 3 is never charged before slot 2', () => {
      // Enough for the EXPENSIVE slot but not the cheap one would let an
      // out-of-order unlock through, if there were one. There is not: the
      // second unlock is priced at COSTS[1] only after the first is bought.
      const store = millStore(0);
      store.addCoins(COSTS[0]!);
      expect(store.unlockMillSlot(0)).toBe(true);
      // Slot 3 costs COSTS[1], which the purse is now short of.
      expect(store.getState().coins).toBeLessThan(COSTS[1]!);
      expect(store.unlockMillSlot(0)).toBe(false);
      expect(unlockedSlotsFor(store.getState(), 'flour_mill')).toBe(2);
    });

    it('refuses (unmutated) on too few coins', () => {
      const store = millStore(0);
      // One coin short of the first slot - the only thing standing in the way.
      store.addCoins(COSTS[0]! - 1 - store.getState().coins);
      const coins = store.getState().coins;
      expect(coins).toBe(COSTS[0]! - 1);
      expect(store.unlockMillSlot(0)).toBe(false);
      expect(store.getState().coins).toBe(coins);
      expect(unlockedSlotsFor(store.getState(), 'flour_mill')).toBe(1);
    });

    it('refuses (unmutated) once every slot is unlocked', () => {
      const store = fullMillStore(0);
      store.addCoins(1_000_000); // far more than any price
      const coins = store.getState().coins;
      expect(store.unlockMillSlot(0)).toBe(false);
      expect(store.getState().coins).toBe(coins);
      expect(unlockedSlotsFor(store.getState(), 'flour_mill')).toBe(RECIPE.slots);
    ...
    it('refuses an out-of-range index and a building with no recipe', () => {
      const store = millStore(0);
      store.addCoins(COSTS[0]!);
      const coins = store.getState().coins;
      expect(store.unlockMillSlot(1)).toBe(false);
      expect(store.unlockMillSlot(-1)).toBe(false);
      expect(store.getState().coins).toBe(coins);

      const recipeless = millStore(0);
      recipeless.addCoins(COSTS[0]!);
      const spy = vi.spyOn(BUILDINGS, 'flour_mill', 'get').mockReturnValue({
        ...BUILDINGS.flour_mill,
        milling: undefined,
      });
      expect(recipeless.unlockMillSlot(0)).toBe(false);
      expect(unlockedSlotsFor(recipeless.getState(), 'flour_mill')).toBe(1);
      spy.mockRestore();
    });

    it('startMilling caps on unlockedSlots, not on the def slot count', () => {
      // Wheat for a full mill, but only the one slot bought.
      const store = millStore(RECIPE.inputCount * RECIPE.slots);
      expect(store.startMilling(0)).toBe(true);
      expect(store.startMilling(0)).toBe(false);
      expect(store.getState().buildings[0]!.batches).toHaveLength(1);

      // Buying a slot makes room immediately - no reload, no re-derivation.
      store.addCoins(COSTS[0]!);
      expect(store.unlockMillSlot(0)).toBe(true);
      expect(store.startMilling(0)).toBe(true);
    ...
    it('devUnlockMillSlot skips the coin gate but keeps the sequential cap', () => {
      const store = millStore(0);
      const coins = store.getState().coins;
      // A post-tutorial purse is nowhere near the first slot's price, so a
      // successful dev unlock can only be the coin gate being skipped.
      expect(coins).toBeLessThan(COSTS[0]!);
      for (let i = 1; i < RECIPE.slots; i++) expect(store.devUnlockMillSlot(0)).toBe(true);
      expect(unlockedSlotsFor(store.getState(), 'flour_mill')).toBe(RECIPE.slots);
      expect(store.getState().coins).toBe(coins); // free
      // Never past the def's own slot count, dev or not.
      expect(store.devUnlockMillSlot(0)).toBe(false);
      expect(unlockedSlotsFor(store.getState(), 'flour_mill')).toBe(RECIPE.slots);
    });
  });
  ...
  describe('collectMilling', () => {
    ...
    it('collects one batch out of several, leaving the rest in flight', () => {
      const store = fullMillStore(RECIPE.inputCount * RECIPE.slots);
      expect(store.startMilling(0)).toBe(true);
      advanceTime(RECIPE.batchMs);
  ...
  describe('millSlots (T4.2b)', () => {
    /**
     * A bare placement carrying exactly `batches`. Paid capacity is no longer a
     * placement field (U3a) - `millSlots` takes it as its 4th arg, defaulting to
     * the full mill, so the batch cases here are not shadowed by the lock (the
     * lock's own tests below pass an explicit count).
     */
    ...
    it('marks every slot at or past unlockedSlots as locked, at its own price', () => {
      // A fresh mill: slot 1 usable, slots 2 and 3 priced by slotUnlockCosts.
      const views = millSlots(placementWith([]), RECIPE, 1_000_000, 1);
      expect(views).toHaveLength(RECIPE.slots);
      expect(views[0]).toEqual({ kind: 'empty' });
      for (let index = 1; index < RECIPE.slots; index++) {
        // The costs list prices the slots PAST the first, so slot index i is
        // priced at slotUnlockCosts[i - 1].
        expect(views[index]).toEqual({
          kind: 'locked',
          cost: RECIPE.slotUnlockCosts[index - 1],
        });
      }
    });

    it('a locked slot stays locked even with a batch sitting behind it', () => {
      // Cannot happen through the store (startMilling caps on unlockedSlots),
      // but the derivation must not leak a locked slot's contents whatever the
      // batch list says.
      const nowMs = 5_000_000;
      const batches = Array.from({ length: RECIPE.slots }, () => ({
        startedAt: nowMs - RECIPE.batchMs,
      }));
      const views = millSlots(placementWith(batches), RECIPE, nowMs, 1);
      expect(views[0]).toEqual({ kind: 'ready', batchIndex: 0 });
      for (let index = 1; index < RECIPE.slots; index++) {
        expect(views[index]!.kind).toBe('locked');
      }
    });

    it('shows no locked slot once every slot is bought', () => {
      const views = millSlots(placementWith([]), RECIPE, 1_000_000, RECIPE.slots);
      expect(views.every((view) => view.kind === 'empty')).toBe(true);
    });
  ...
  describe('offline milling', () => {
    ...
    it('batches survive a save/load round trip untouched', () => {
      const storage = makeStorage();
      const store = fullMillStore(RECIPE.inputCount * 2, storage);
      expect(store.startMilling(0)).toBe(true);
      expect(store.startMilling(0)).toBe(true);
  ...
  describe('anti-rollback clamp', () => {
    it('a batch startedAt in the future of the real clock is clamped down on load', () => {
      ...
      saved.buildings = [
        { type: 'flour_mill', col: -3, row: 0, batches: [{ startedAt: future }], unlockedSlots: 1 },
      ];
      const storage = makeStorage({ [SAVE_KEY]: JSON.stringify(saved) });
      const store = new GameStateStore({ storage });
      const before = Date.now();
    ...
    it('a past-stamped batch is left exactly alone', () => {
      ...
      saved.buildings = [
        { type: 'flour_mill', col: -3, row: 0, batches: [{ startedAt: past }], unlockedSlots: 1 },
      ];
      const storage = makeStorage({ [SAVE_KEY]: JSON.stringify(saved) });
      const store = new GameStateStore({ storage });
      store.load();
  ...
  describe('v26 -> v27 migration', () => {
    it('adds flipped: false to every building AND both anchors (whole-state re-pin)', () => {
      ...
      expect(store.getState()).toEqual({
        ...
        // U3a: v31ToV32 strips the interim `unlockedSlots: 1` back off; a 1 is
        // the born-with default, so no buildingSlotUnlocks key is written.
        buildings: [
          { type: 'flour_mill', col: -3, row: 0, batches: [], flipped: false },
          { type: 'bakery', col: -5, row: -4, batches: [], flipped: false },
        ],
      });
      expect(console.warn).not.toHaveBeenCalled();
    });
...
describe('paths (T4.12)', () => {
  /** A post-tutorial save at the CURRENT version, so nothing migrates on load. */
  function savedWithPaths(overrides: Partial<GameStateData> = {}): GameStateData {
    const saved = createDefaultState(28);
    saved.onboarding = { completed: true, step: 0, progress: 0, progressB: 0 };
    return { ...saved, ...overrides };
  }
  ...
  describe('migration v27 -> v28', () => {
    it('a v27 save (no paths field) gains an empty paths list, no warnings', () => {
      const { paths, ...v27Save } = createDefaultState(27);
      void paths;
      const storage = makeStorage({ [SAVE_KEY]: JSON.stringify(v27Save) });
      const store = new GameStateStore({ storage });
      store.load();
    ...
    it('a full-chain v1 save lands at version 28 with no paths', () => {
      const raw = { ...createDefaultState(1), coins: 10 };
      const storage = makeStorage({ [SAVE_KEY]: JSON.stringify(raw) });
      const store = new GameStateStore({ storage });
      store.load();
      expect(store.getState().version).toBe(33);
  ...
  describe('validation', () => {
    it('accepts a valid paths array', () => {
      const state = savedWithPaths({ paths: [{ col: 2, row: 3, tier: 'gravel' }] });
      expect(isValidState(state, 28)).toBe(true);
    });

    it('rejects a malformed paths array', () => {
      const malformed: unknown[] = [
        // Not an object.
        'gravel',
        // Unknown tier - would render a missing atlas frame.
        { col: 0, row: 0, tier: 'obsidian' },
        // Non-integer / out-of-bounds coordinates.
        { col: 0.5, row: 0, tier: 'gravel' },
        { col: PLOT_GRID_COORD_MAX + 1, row: 0, tier: 'gravel' },
        { col: PLOT_GRID_COORD_MIN - 1, row: 0, tier: 'gravel' },
        // Missing tier.
        { col: 0, row: 0 },
      ];
      for (const entry of malformed) {
        const state = { ...savedWithPaths(), paths: [entry] };
        expect(isValidState(state, 28)).toBe(false);
      }
      // The field itself must be an array.
      expect(isValidState({ ...savedWithPaths(), paths: {} }, 28)).toBe(false);
      // At most one tile per (col, row).
      const duped = savedWithPaths({
        paths: [
          { col: 1, row: 1, tier: 'gravel' },
          { col: 1, row: 1, tier: 'gravel' },
        ],
      });
      expect(isValidState(duped, 28)).toBe(false);
    });
    ...
    it('round-trips a painted tile through save and load', () => {
      ...
      const reloaded = new GameStateStore({ storage });
      reloaded.load();
      expect(reloaded.getState().paths).toEqual([{ col: -3, row: 5, tier: 'gravel' }]);
      expect(console.warn).not.toHaveBeenCalled();
    });
  });
...
/**
 * The Shed pipeline (U1): Shop --buyToShed--> shed --placeFromShed--> farm, and
 * farm --putAwayToShed--> shed back again.
 *
 * Two invariants run through every case here, and each is asserted rather than
 * assumed: NOTHING IS DESTROYED (a count only ever moves between the shed and a
 * placed collection), and NO CURRENCY MOVES outside `buyToShed`.
 */
describe('the Shed (U1, schema v29; warehouse cutover U2a, schema v30)', () => {
  function makeShedStore(storage: SaveStorage | null = null): GameStateStore {
    const store = new GameStateStore({ storage });
    completeOnboarding(store);
    return store;
  }

  /** Coins + moondust, for asserting a reducer moved neither. */
  function purse(store: GameStateStore): { coins: number; moondust: number } {
    const { coins, moondust } = store.getState();
    return { coins, moondust };
  }

  describe('schema v29 -> v30', () => {
    it('a v28 save gains an empty shedInventory and nothing else changes', () => {
      const { shedInventory, ...v28Save } = createDefaultState(28);
      void shedInventory;
      const storage = makeStorage({ [SAVE_KEY]: JSON.stringify(v28Save) });
      const store = new GameStateStore({ storage });
      store.load();
    ...
    it('merges an existing decoration warehouse into the shed, trophies included, losing nothing', () => {
      const { shedInventory, ...rest } = createDefaultState(28);
      void shedInventory;
      // DELIBERATE RE-PIN (U2a): U1 pinned this as a MIRROR that left trophies
      // behind (they had no catalog id then) and kept `warehouse` live. Both
      // halves flipped: trophies are catalog items now, so they merge like any
      // other decoration, and the field itself is deleted.
      const v28Save = {
        ...rest,
        warehouse: { decor_gnome: 2, decor_bench: 1, trophy_moonwell: 1 },
      } as unknown as Record<string, unknown>;
      const storage = makeStorage({ [SAVE_KEY]: JSON.stringify(v28Save) });
      const store = new GameStateStore({ storage });
      store.load();
      ...
      expect('warehouse' in (store.getState() as unknown as Record<string, unknown>)).toBe(false);
      expect(console.warn).not.toHaveBeenCalled();
    });

    /**
     * The real-world v29 save: the shed holds U1's one-time MIRROR while the
     * warehouse kept moving underneath it (the decor shop bought into the
     * warehouse alone), so the two DIVERGED. The merge has to take the
     * warehouse's number wholesale - adding would double-count the mirrored
     * items, and keeping the shed's would silently roll back every purchase
     * made since U1.
     */
    it('a v29 save with a DIVERGED warehouse merges to the warehouse counts, dropping the stale mirror', () => {
      const v29Save = {
        ...createDefaultState(29),
        version: 29,
        // The stale U1 mirror: 1 gnome (bought a second one since), 4 benches
        // (placed three since), and no trace of the later fence purchase.
        shedInventory: { decor_gnome: 1, decor_bench: 4 },
        // The live, authoritative warehouse at the moment of the cutover.
        warehouse: {
          decor_gnome: 2,
          decor_bench: 1,
          decor_fence: 3,
      ...
      const storage = makeStorage({ [SAVE_KEY]: JSON.stringify(v29Save) });
      const store = new GameStateStore({ storage });
      store.load();
      const state = store.getState();
      // Whole-state pin: version bumped, the two records collapsed into one,
      // and NOTHING else moved - in particular no currency.
      const { shedInventory, warehouse, ...untouched } = v29Save;
      void shedInventory;
      void warehouse;
      ...
      expect('warehouse' in (state as unknown as Record<string, unknown>)).toBe(false);
      expect(console.warn).not.toHaveBeenCalled();
    });

    it('a fresh save has no warehouse field at all', () => {
      const store = new GameStateStore({ storage: null });
      const state = store.getState() as unknown as Record<string, unknown>;
      expect('warehouse' in state).toBe(false);
      expect(store.getState().shedInventory).toEqual({});
    ...
    it('round-trips shed counts through save and load', () => {
      const storage = makeStorage();
      const store = makeShedStore(storage);
      store.devGrantToShed('decor_bench', 3);
      store.devGrantToShed('gravel', 12);
      const reloaded = new GameStateStore({ storage });
      reloaded.load();
      expect(reloaded.getState().shedInventory).toEqual({ decor_bench: 3, gravel: 12 });
      expect(console.warn).not.toHaveBeenCalled();
    });

    it('validation rejects an unknown id, a negative count, and a zero count', () => {
      const base = createDefaultState(31);
      expect(isValidState({ ...base, shedInventory: {} }, 31)).toBe(true);
      expect(isValidState({ ...base, shedInventory: { decor_bench: 2 } }, 31)).toBe(true);
      // Unknown ids are rejected, not ignored - a shed count is spendable
      // ownership, so an item this build cannot place makes the save unusable.
      expect(isValidState({ ...base, shedInventory: { not_an_item: 1 } }, 31)).toBe(false);
      // DELIBERATE RE-PIN (U2a): trophies BECAME catalog items, so `CATALOG_IDS`
      // now admits them and this same rule accepts what it used to reject -
      // `isShedInventoryRecord` itself needed no change.
      expect(isValidState({ ...base, shedInventory: { trophy_moonwell: 1 } }, 31)).toBe(true);
      expect(isValidState({ ...base, shedInventory: { decor_bench: -1 } }, 31)).toBe(false);
      expect(isValidState({ ...base, shedInventory: { decor_bench: 0 } }, 31)).toBe(false);
      expect(isValidState({ ...base, shedInventory: { decor_bench: 1.5 } }, 31)).toBe(false);
    });
  });

  describe('buyToShed', () => {
    it('charges coins for a coin item and banks the count', () => {
      const store = makeShedStore();
      store.addCoins(10_000);
      const before = purse(store);
      // Price read from the catalog (which reads the registry) - never a
      // literal here, so a rebalance flows through silently.
      const bench = findCatalogItem('decor_bench')!;
      expect(store.buyToShed('decor_bench', 2)).toBe(true);
      expect(store.getState().coins).toBe(before.coins - bench.price * 2);
      expect(store.getState().moondust).toBe(before.moondust);
      expect(store.shedCount('decor_bench')).toBe(2);
    });

    it('charges moondust for a moondust item, leaving coins alone', () => {
      const store = makeShedStore();
      store.addMoondust(500);
      const before = purse(store);
      const gnome = findCatalogItem('decor_gnome')!;
      expect(gnome.currency).toBe('moondust');
      expect(store.buyToShed('decor_gnome', 3)).toBe(true);
      expect(store.getState().moondust).toBe(before.moondust - gnome.price * 3);
      expect(store.getState().coins).toBe(before.coins);
      expect(store.shedCount('decor_gnome')).toBe(3);
    });

    it('refuses below the item unlockLevel, mutating nothing', () => {
      const store = makeShedStore();
      store.addCoins(100_000);
      const mill = findCatalogItem('flour_mill')!;
      expect(mill.unlockLevel).toBeGreaterThan(1);
      store.setLevel(mill.unlockLevel - 1);
      const before = purse(store);
      expect(store.buyToShed('flour_mill')).toBe(false);
      expect(purse(store)).toEqual(before);
      expect(store.getState().shedInventory).toEqual({});
      // At the gate it goes through.
      store.setLevel(mill.unlockLevel);
      expect(store.buyToShed('flour_mill')).toBe(true);
      expect(store.shedCount('flour_mill')).toBe(1);
    });

    it('refuses on insufficient funds for the FULL quantity - no partial buy', () => {
      const store = makeShedStore();
      const bench = findCatalogItem('decor_bench')!;
      // Topped up to exactly enough for two, then asked for three.
      store.addCoins(bench.price * 2 - store.getState().coins);
      const before = purse(store);
      expect(store.buyToShed('decor_bench', 3)).toBe(false);
      expect(purse(store)).toEqual(before);
      expect(store.getState().shedInventory).toEqual({});
      expect(store.buyToShed('decor_bench', 2)).toBe(true);
      expect(store.shedCount('decor_bench')).toBe(2);
    ...
    it('refuses an unknown id and a non-positive or fractional quantity', () => {
      const store = makeShedStore();
      store.addCoins(10_000);
      const before = purse(store);
      expect(store.buyToShed('not_an_item')).toBe(false);
      expect(store.buyToShed('decor_bench', 0)).toBe(false);
      expect(store.buyToShed('decor_bench', -1)).toBe(false);
      expect(store.buyToShed('decor_bench', 1.5)).toBe(false);
      expect(purse(store)).toEqual(before);
      expect(store.getState().shedInventory).toEqual({});
    });

    it('buys a free path tier without charging, and a priced one at cost per tile', () => {
      const store = makeShedStore();
      store.addCoins(10_000);
      const before = purse(store);
      // dirt is the free rung; gravel is priced per TILE.
      expect(findCatalogItem('dirt')!.price).toBe(0);
      expect(store.buyToShed('dirt', 5)).toBe(true);
      expect(purse(store)).toEqual(before);
      const gravel = findCatalogItem('gravel')!;
      expect(store.buyToShed('gravel', 4)).toBe(true);
      expect(store.getState().coins).toBe(before.coins - gravel.price * 4);
      expect(store.getState().shedInventory).toEqual({ dirt: 5, gravel: 4 });
    });

    // Split-budget cap moved into buyToShed (U2b): qty-aware, no partial buy,
    // boundary exact. Fences and non-fence purchasable decor cap independently;
    // trophies and non-decor items (paths, buildings) are exempt.
    it('refuses a non-fence decor buy that would exceed MAX_DECOR_ITEMS, exact at the cap', () => {
      const store = makeShedStore();
      store.addCoins(10_000_000);
      // Seat the shed one buy short of the cap, then prove the boundary.
      store.devGrantToShed('decor_bench', MAX_DECOR_ITEMS - 2);
      const snapshot = JSON.parse(store.exportSave()) as unknown;
      // 48 + 3 = 51 > 50: refused whole, nothing mutated.
      expect(store.buyToShed('decor_bench', 3)).toBe(false);
      expect(JSON.parse(store.exportSave())).toEqual(snapshot);
      // 48 + 2 = 50: exactly the cap, accepted.
      expect(store.buyToShed('decor_bench', 2)).toBe(true);
      expect(store.shedCount('decor_bench')).toBe(MAX_DECOR_ITEMS);
      // At the cap, even one more is refused.
      expect(store.buyToShed('decor_bench', 1)).toBe(false);
    });

    it('refuses a fence buy that would exceed MAX_FENCES, exact at the cap - decor budget independent', () => {
      const store = makeShedStore();
      store.addCoins(10_000_000);
      store.devGrantToShed('decor_fence', MAX_FENCES - 2);
      const snapshot = JSON.parse(store.exportSave()) as unknown;
      expect(store.buyToShed('decor_fence', 3)).toBe(false);
      expect(JSON.parse(store.exportSave())).toEqual(snapshot);
      expect(store.buyToShed('decor_fence', 2)).toBe(true);
      expect(store.shedCount('decor_fence')).toBe(MAX_FENCES);
      // A full fence budget does not block the decor budget.
      expect(store.buyToShed('decor_bench', 1)).toBe(true);
    });

    it('counts placed decorations toward the cap, and exempts trophies', () => {
      const store = makeShedStore();
      store.addCoins(10_000_000);
      // 40 placed + 8 shed = 48 non-fence purchasable; trophies do not count.
      store.devGrantToShed('decor_bench', 40);
      for (let i = 0; i < 40; i++) expect(store.placeFromShed('decor_bench')).not.toBe(false);
      store.devGrantToShed('decor_bench', 8);
      store.getState().shedInventory.trophy_moonwell = 5;
      expect(store.getState().decorations).toHaveLength(40);
      // 48 + 2 = 50 exact: accepted despite the 5 trophies sharing the shed.
      expect(store.buyToShed('decor_bench', 2)).toBe(true);
      // 50 + 1: over the cap.
      expect(store.buyToShed('decor_bench', 1)).toBe(false);
    });
  ...
  describe('placeFromShed / putAwayToShed round trips', () => {
    it('refuses to place at count 0, mutating nothing', () => {
      const store = makeShedStore();
      const before = purse(store);
      expect(store.placeFromShed('decor_bench')).toBe(false);
      expect(store.placeFromShed('not_an_item')).toBe(false);
      expect(store.getState().decorations).toEqual([]);
      expect(purse(store)).toEqual(before);
    });

    it('a decoration placed from the shed is identical to one placed from the warehouse', () => {
      const shedStore = makeShedStore();
      shedStore.devGrantToShed('decor_bench', 1);
      expect(shedStore.placeFromShed('decor_bench')).toBe(0);

      // The SAME item through today's flow, for comparison - the two placements
      ...
      // The key is deleted at 0, never left sitting at 0.
      expect(shedStore.getState().shedInventory).toEqual({});
    });

    it('decor: put away restores the count and a re-place restores position, scale and flip', () => {
      const store = makeShedStore();
      const before = purse(store);
      store.devGrantToShed('decor_bench', 1);
      expect(store.placeFromShed('decor_bench')).toBe(0);
      // Arrange it somewhere specific, through the existing clamp authority.
      expect(store.setDecorationTransform(0, 400, 1000, 0.5, true)).toBe(true);
      const placed = { ...store.getState().decorations[0]! };

      const result = store.putAwayToShed({ category: 'decor', index: 0 });
      expect(result).not.toBeNull();
      expect(result!.itemId).toBe('decor_bench');
      expect(store.getState().decorations).toEqual([]);
      expect(store.shedCount('decor_bench')).toBe(1);

      // Feeding the result straight back is the exact inverse.
      expect(store.placeFromShed(result!.itemId, result!.options)).toBe(0);
      expect(store.getState().decorations[0]).toEqual(placed);
      expect(store.shedCount('decor_bench')).toBe(0);
      // Zero currency movement across the whole round trip.
      expect(purse(store)).toEqual(before);
    });

    it('a building placed from the shed matches one from buyBuilding, and round-trips', () => {
      const shedStore = makeShedStore();
      shedStore.devGrantToShed('flour_mill', 1);
      expect(shedStore.placeFromShed('flour_mill')).toBe(0);

      const legacyStore = makeShedStore();
      legacyStore.addCoins(100_000);
      legacyStore.setLevel(BUILDINGS.flour_mill.unlockLevel);
      expect(legacyStore.buyBuilding('flour_mill')).toBe(true);

      expect(shedStore.getState().buildings).toEqual(legacyStore.getState().buildings);

      const before = purse(shedStore);
      const result = shedStore.putAwayToShed({ category: 'building', index: 0 });
      expect(result).not.toBeNull();
      expect(result!.itemId).toBe('flour_mill');
      expect(shedStore.getState().buildings).toEqual([]);
      expect(shedStore.shedCount('flour_mill')).toBe(1);
      expect(shedStore.placeFromShed(result!.itemId, result!.options)).toBe(0);
      expect(shedStore.getState().buildings).toEqual(legacyStore.getState().buildings);
      expect(purse(shedStore)).toEqual(before);
    });

    it('building: an explicit anchor is judged by isBuildingAnchorFree; flip survives', () => {
      const store = makeShedStore();
      store.devGrantToShed('flour_mill', 1);
      // (0, 0) puts the 2x2 footprint straight onto the starting plot block.
      expect(isBuildingAnchorFree(store.getState(), 'flour_mill', 0, 0)).toBe(false);
      expect(store.placeFromShed('flour_mill', { col: 0, row: 0 })).toBe(false);
      // Refused without spending the count.
      expect(store.shedCount('flour_mill')).toBe(1);
      expect(store.getState().buildings).toEqual([]);

      // The open south field, the same anchor the moveStructure matrix uses.
      expect(isBuildingAnchorFree(store.getState(), 'flour_mill', 6, 7)).toBe(true);
      expect(store.placeFromShed('flour_mill', { col: 6, row: 7, flip: true })).toBe(0);
      const placed = store.getState().buildings[0]!;
      expect(placed.col).toBe(6);
      expect(placed.row).toBe(7);
      expect(placed.flipped).toBe(true);
    });

    it('building: refuses a second placement of a non-allowMultiple item', () => {
      const store = makeShedStore();
      store.devGrantToShed('flour_mill', 2);
      expect(findCatalogItem('flour_mill')!.allowMultiple).toBe(false);
      expect(store.placeFromShed('flour_mill')).toBe(0);
      expect(store.placeFromShed('flour_mill')).toBe(false);
      // The second count is still in the shed, not burned.
      expect(store.shedCount('flour_mill')).toBe(1);
      expect(store.getState().buildings).toHaveLength(1);
    });

    it('building: refuses to put away one with batches in flight, mutating nothing', () => {
      const store = makeShedStore();
      store.devGrantToShed('flour_mill', 1);
      expect(store.placeFromShed('flour_mill')).toBe(0);
      const recipe = BUILDINGS.flour_mill.milling!;
      store.getState().inventory[MILL_INPUT_CROP] = recipe.inputCount;
      expect(store.startMilling(0)).toBe(true);
      expect(store.putAwayToShed({ category: 'building', index: 0 })).toBeNull();
      expect(store.getState().buildings).toHaveLength(1);
      expect(store.shedCount('flour_mill')).toBe(0);
    });

    it('RE-PIN (U3a): paid slot capacity is per-TYPE, so a put-away then re-place keeps it for free', () => {
      const store = makeShedStore();
      store.devGrantToShed('flour_mill', 1);
      expect(store.placeFromShed('flour_mill')).toBe(0);
      expect(store.devUnlockMillSlot(0)).toBe(true);
      ...
      const result = store.putAwayToShed({ category: 'building', index: 0 })!;
      // Capacity is no longer carried on the options - it lives in the per-type
      // map, untouched by put-away (U3a: simpler than U1's per-instance carry).
      expect('unlockedSlots' in result.options).toBe(false);
      expect(store.getState().buildingSlotUnlocks.flour_mill).toBe(2);

      // Re-placing with the returned options keeps the capacity...
      expect(store.placeFromShed(result.itemId, result.options)).toBe(0);
      expect(unlockedSlotsFor(store.getState(), 'flour_mill')).toBe(2);

      // ...and so does a bare re-place with NO options: capacity is the TYPE's,
      // not the instance's, so nothing has to be threaded through to keep it.
      expect(store.putAwayToShed({ category: 'building', index: 0 })).not.toBeNull();
      expect(store.placeFromShed('flour_mill')).toBe(0);
      expect(unlockedSlotsFor(store.getState(), 'flour_mill')).toBe(2);
    });

    it('path: place and put away move a tile between farm and shed, free both ways', () => {
      const store = makeShedStore();
      const before = purse(store);
      store.devGrantToShed('gravel', 2);
      expect(store.placeFromShed('gravel', { col: 2, row: 2 })).toBe(0);
      expect(store.getState().paths).toEqual([{ col: 2, row: 2, tier: 'gravel' }]);
      ...
      const result = store.putAwayToShed({ category: 'path', col: 2, row: 2 })!;
      expect(result.itemId).toBe('gravel');
      expect(result.options).toEqual({ col: 2, row: 2 });
      expect(store.getState().paths).toEqual([]);
      expect(store.shedCount('gravel')).toBe(2);
      expect(purse(store)).toEqual(before);
    });

    it('path: painting over another tier banks the displaced tier - nothing is destroyed', () => {
      const store = makeShedStore();
      store.devGrantToShed('moonstone', 1);
      store.devGrantToShed('dirt', 1);
      expect(store.placeFromShed('moonstone', { col: 3, row: 3 })).toBe(0);
      expect(store.shedCount('moonstone')).toBe(0);
      // Dirt over moonstone: the moonstone goes back in the shed rather than
      // evaporating.
      expect(store.placeFromShed('dirt', { col: 3, row: 3 })).toBe(0);
      expect(store.getState().paths).toEqual([{ col: 3, row: 3, tier: 'dirt' }]);
      expect(store.shedCount('moonstone')).toBe(1);
      expect(store.shedCount('dirt')).toBe(0);
    });

    it('path: refuses without coordinates, off the grid, or onto its own tier', () => {
      const store = makeShedStore();
      store.devGrantToShed('gravel', 3);
      expect(store.placeFromShed('gravel')).toBe(false);
      expect(store.placeFromShed('gravel', { col: 2 })).toBe(false);
      expect(store.placeFromShed('gravel', { col: PLOT_GRID_COORD_MAX + 1, row: 0 })).toBe(false);
      expect(store.shedCount('gravel')).toBe(3);
      expect(store.placeFromShed('gravel', { col: 2, row: 2 })).toBe(0);
      // Same tier on the same tile: nothing to do, and it must not burn a count.
      expect(store.placeFromShed('gravel', { col: 2, row: 2 })).toBe(false);
      expect(store.shedCount('gravel')).toBe(2);
    });

    it('putAwayToShed refuses a bad reference, mutating nothing', () => {
      const store = makeShedStore();
      expect(store.putAwayToShed({ category: 'decor', index: 0 })).toBeNull();
      expect(store.putAwayToShed({ category: 'building', index: 3 })).toBeNull();
      expect(store.putAwayToShed({ category: 'path', col: 9, row: 9 })).toBeNull();
    });

    it('putAwayToShed accepts a TROPHY and banks it in the shed', () => {
      const store = makeShedStore();
      // DELIBERATE RE-PIN (U2a): U1 pinned a trophy as REFUSED here, because a
      // trophy had no catalog id and stayed on the warehouse path. Trophies are
      ...
      expect(store.getState().decorations).toHaveLength(0);
      expect(store.getState().shedInventory).toEqual({ trophy_moonwell: 1 });
    });
  });

  describe('existing flows are untouched', () => {
    it('buyDecoration now buys straight into the shed', () => {
      const store = makeShedStore();
      store.addCoins(10_000);
      // DELIBERATE RE-PIN (U2a): U1 pinned this as buying into the `warehouse`
      // and leaving the shed EMPTY, because the two stores were still parallel.
      // The warehouse is gone, so the purchase lands in the one store.
      expect(store.buyDecoration('decor_bench')).toBe(true);
      expect(store.getState().shedInventory).toEqual({ decor_bench: 1 });
    });

    it('buyDecoration refuses a TROPHY, which the catalog prices at 0', () => {
      const store = makeShedStore();
      store.addCoins(10_000);
      // Trophies joined the catalog in U2a at an inert price of 0. The decor
      // shop's own DECOR_ITEMS lookup is what stops that from becoming a free
      // trophy, and it runs before the delegation to `buyToShed`.
      expect(store.buyDecoration('trophy_moonwell')).toBe(false);
      expect(store.getState().shedInventory).toEqual({});
    });
    ...
    it('buyBuilding still charges coins and places at the default anchor', () => {
      const store = makeShedStore();
      store.addCoins(100_000);
      store.setLevel(BUILDINGS.flour_mill.unlockLevel);
      const before = purse(store);
      expect(store.buyBuilding('flour_mill')).toBe(true);
      expect(store.getState().coins).toBe(before.coins - BUILDINGS.flour_mill.price);
      expect(store.getState().buildings[0]!.col).toBe(BUILDINGS.flour_mill.defaultAnchor.col);
      expect(store.getState().shedInventory).toEqual({});
    });
  });
});

describe('real migration v31 -> v32 (per-type building slots, U3a)', () => {
  it('moves a placed mill unlockedSlots into buildingSlotUnlocks and strips the field (whole-state re-pin)', () => {
    // A genuine v31 save: placements still carry `unlockedSlots`, and there is
    // no buildingSlotUnlocks map. The mill has 3 paid slots; the bakery has the
    // born-with 1.
    const saved = createDefaultState(32) as unknown as Record<string, unknown>;
    saved.version = 31;
    delete saved.buildingSlotUnlocks;
    saved.buildings = [
      { type: 'flour_mill', col: -3, row: 0, batches: [], unlockedSlots: 3, flipped: false },
  ...
  it('a fresh save has an empty map, and a v1 full-chain save lands with one too', () => {
    expect(createDefaultState(32).buildingSlotUnlocks).toEqual({});
    const saved = createDefaultState(1) as unknown as Record<string, unknown>;
    const storage = makeStorage({ [SAVE_KEY]: JSON.stringify(saved) });
    const store = new GameStateStore({ storage });
    store.load();
  ...
  it('buildingSlotUnlocks round-trips through export/import', () => {
    const store = new GameStateStore({ storage: null });
    completeOnboarding(store);
    expect(store.devBuildBuilding('flour_mill')).toBe(true);
    expect(store.devUnlockMillSlot(0)).toBe(true);
    expect(store.getState().buildingSlotUnlocks).toEqual({ flour_mill: 2 });
    const reloaded = new GameStateStore({ storage: null });
    expect(reloaded.importSave(store.exportSave())).toBe(true);
...
describe('edit-session undo stack (U3a, model only)', () => {
  /** A post-onboarding, expanded store with shed stock to arrange. */
  function arrangeStore(): GameStateStore {
    const store = new GameStateStore({ storage: null });
    completeOnboarding(store);
    markExpanded(store);
    store.devGrantToShed('decor_bench', 5);
    store.devGrantToShed('flour_mill', 1);
    return store;
  }

  it('records nothing and undoes nothing outside a session', () => {
  ...
  it('undo of a decor place puts the instance back, whole-state byte-identical', () => {
    const store = arrangeStore();
  ...
  it('LIFO across a place -> move -> flip session restores each step byte-identically', () => {
    const store = arrangeStore();
    const s0 = JSON.parse(JSON.stringify(store.getState())) as unknown;
    store.beginEditSession();
    expect(store.placeFromShed('decor_bench')).toBe(0);
    const s1 = JSON.parse(JSON.stringify(store.getState())) as unknown;
    expect(store.setDecorationTransform(0, 600, 900, 0.7, false)).toBe(true); // move + scale
    const s2 = JSON.parse(JSON.stringify(store.getState())) as unknown;
    expect(store.setDecorationTransform(0, 600, 900, 0.7, true)).toBe(true); // flip
    expect(store.editUndoDepth()).toBe(3);
    expect(store.undoEditAction()).toBe(true); // undo flip -> s2
    expect(store.getState()).toEqual(s2);
    expect(store.undoEditAction()).toBe(true); // undo move -> s1
    expect(store.getState()).toEqual(s1);
    expect(store.undoEditAction()).toBe(true); // undo place -> s0
    expect(store.getState()).toEqual(s0);
    expect(store.editUndoDepth()).toBe(0);
    expect(store.undoEditAction()).toBe(false); // empty stack
  });

  it('records one entry each for building move + flip and undoes them', () => {
    const store = arrangeStore();
    expect(store.placeFromShed('flour_mill')).toBe(0);
    const anchor = {
      col: store.getState().buildings[0]!.col,
      row: store.getState().buildings[0]!.row,
    };
    store.beginEditSession();
    expect(store.moveBuilding(0, 6, 6)).toBe(true);
    expect(store.flipBuilding(0)).toBe(true);
    expect(store.editUndoDepth()).toBe(2);
    expect(store.undoEditAction()).toBe(true); // un-flip
    expect(store.getState().buildings[0]!.flipped).toBe(false);
    expect(store.undoEditAction()).toBe(true); // un-move
    expect(store.getState().buildings[0]!.col).toBe(anchor.col);
  ...
  it('records one entry each for structure move + flip and undoes them', () => {
    ...
    store.beginEditSession();
    // A move onto the structure's own anchor is a valid commit (records an
    // inverse); the flip is a real change.
    expect(store.moveStructure('farmhouse', fh.col, fh.row)).toBe(true);
    expect(store.flipStructure('farmhouse')).toBe(true);
    expect(store.editUndoDepth()).toBe(2);
    expect(store.undoEditAction()).toBe(true);
    expect(store.undoEditAction()).toBe(true);
  ...
  it('undo of a put-away places the instance back exactly where it was', () => {
    const store = arrangeStore();
    store.beginEditSession();
    expect(store.placeFromShed('decor_bench')).toBe(0);
    expect(store.setDecorationTransform(0, 600, 900, 0.7, true)).toBe(true);
    const placed = JSON.parse(JSON.stringify(store.getState().decorations[0])) as unknown;
    // Fresh session so the put-away is the only entry on the stack.
    store.endEditSession();
    store.beginEditSession();
    expect(store.putAwayToShed({ category: 'decor', index: 0 })).not.toBeNull();
    expect(store.getState().decorations).toHaveLength(0);
    expect(store.editUndoDepth()).toBe(1);
    expect(store.undoEditAction()).toBe(true);
    expect(store.getState().decorations[0]).toEqual(placed);
  ...
  it('delegates and compound places record exactly ONE entry each, never two', () => {
    ...
    expect(store.storeDecoration(0)).toBe(true);
    expect(store.editUndoDepth()).toBe(2);
    // A decor place WITH a transform runs setDecorationTransform internally -
    // that nested call must not add a second entry.
    expect(store.placeFromShed('decor_bench', { x: 600, y: 900, scale: 0.7, flip: true })).toBe(0);
    expect(store.editUndoDepth()).toBe(3);
  });

  it('purchases record nothing inside a session', () => {
    const store = arrangeStore();
    store.setLevel(5);
    store.addCoins(100_000);
    store.beginEditSession();
  ...
  it('endEditSession clears the stack and stops recording', () => {
    const store = arrangeStore();
    store.beginEditSession();
    expect(store.placeFromShed('decor_bench')).toBe(0);
    expect(store.editUndoDepth()).toBe(1);
  ...
  it('a refused inverse returns false, mutates nothing, discards the entry, and the next undo proceeds', () => {
    const store = arrangeStore();
    store.beginEditSession();
    const fh = { ...store.getState().structures.farmhouse };
    // Entry A (bottom): a structure flip, undone LAST.
    expect(store.flipStructure('farmhouse')).toBe(true);
    // Entry B: place a bench (inverse = put that instance away).
    expect(store.placeFromShed('decor_bench')).toBe(0);
    // Entry C (top): put that same bench away (inverse = place it back).
    expect(store.putAwayToShed({ category: 'decor', index: 0 })).not.toBeNull();
    expect(store.editUndoDepth()).toBe(3);

    // Undo C: re-places the bench as a NEW instance.
    expect(store.undoEditAction()).toBe(true);
    const afterReplace = JSON.parse(JSON.stringify(store.getState())) as unknown;
    // Undo B: would put away the ORIGINAL instance, which is gone - refused.
    expect(store.undoEditAction()).toBe(false);
    expect(store.getState()).toEqual(afterReplace); // nothing mutated
    expect(store.editUndoDepth()).toBe(1); // B discarded, A remains
